...
async function resolveWritePolicy(ctx, exec) {
  if (ctx.fs.sandboxMode === void 0) return void 0;
  const service = typeof ctx.get === "function" ? ctx.get("sandboxPolicy") : void 0;
  if (service === void 0 || service === null) {
    throw new Error("office tools: the mounted filesystem confines but ctx.sandboxPolicy is missing");
  }
  if (typeof service.resolve !== "function") return void 0;
  return service.resolve(exec.agent === void 0 ? {} : { session: exec.agent.session });
}
...
async function resolveOfficePath(exec, ctx, rawPath, allowedExts, mustExist, writePolicy) {
...
async function saveOfficeText(exec, ctx, target, text, writePolicy) {
  exec.signal.throwIfAborted();
  await ctx.fs.writeText(target, text, void 0, exec.signal, writePolicy);
  exec.signal.throwIfAborted();
